...
```{r echo=TRUE, warning=FALSE, message=FALSE}
country_frame <- as.data.frame(
  table(data_set$location)
)
colnames(country_frame) <- c("country", "observations")
print(head(country_frame, 20))
print(dim(country_frame)[1])
```
...
data_set %>% 
  select(location, new_cases, year) %>% 
...
```{r echo=TRUE, warning=FALSE, message=FALSE}
remove_cols <- c('people_vaccinated_per_hundred', 'date')
...
```{r echo=TRUE, warning=FALSE, message=FALSE}
country_analysis_frame$proportion_vaccinated <- 
  country_analysis_frame$people_vaccinated/country_analysis_frame$population
country_analysis_frame$population <- NULL
country_analysis_frame$people_vaccinated <- NULL
```
