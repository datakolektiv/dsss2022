...
- The axis above presents the number of non-zero coefficients (predictors) present at the respective level of the $L1$ norm; it is constant (11) in our case since we have chosen to perform Ridge Regression ($\alpha=0$);
...
Then, to estimate the best found Ridge Regression model - without cross-validation across $\lambda$ yet - we need to:
...
Do exactly the same as I did but use the $LASSO$ regression in place of Ridge that I have used. Try to understand what happens and learn as much as you can about the `glmnet()` function. 
