...
```{r echo=TRUE, warning=FALSE, message=FALSE}
plot_frame <- data_set %>% 
  select(continent, year, new_cases) %>% 
...
The mean `gdp_per_capita` per country:
...
There will be some missing data: remove with `complete.cases()`:

```{r echo=TRUE, warning=FALSE, message=FALSE}
gdp_set <- gdp_set[complete.cases(gdp_set), ]
```

Join `gdp_set` to `population_set` to produce `plot_frame`:
...
Missing data again (this is now a by-product of `left_join()`):

```{r echo=TRUE, warning=FALSE, message=FALSE}
plot_frame <- plot_frame[complete.cases(plot_frame), ]
```
...
```{r echo=TRUE, warning=FALSE, message=FALSE}
ggplot(data = plot_frame, 
       aes(x = population_density, y = mean_gdp)) + 
...
```{r echo=TRUE, warning=FALSE, message=FALSE}
ggplot(data = plot_frame, 
       aes(x = log(population_density), y = log(mean_gdp))) + 
  geom_point(size = 1, color = "blue") + 
  geom_point(size = .75, color = "blue") + 
  scale_y_continuous(labels = comma) + 
  scale_x_continuous(labels = comma) + 
